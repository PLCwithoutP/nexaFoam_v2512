...
Foam::autoPtr<ChemistryModel> Foam::basicNeChemistryModel::New
(
...
)
{
    ...
    (
      ...
      + ChemistryModel::ne2TReactionThermo::typeName + ','
      + thermo.thermoName() + ">>"
    );
    ...
    if (!ctorPtr)
    {
        ...
        thisCmpts.append(ChemistryModel::ne2TReactionThermo::typeName);
        thisCmpts.append
        (
            basic2TThermo::splitThermoName(thermo.thermoName(), 5)
        );
        ...
        {
            wordList cmpts(basic2TThermo::splitThermoName(validName, nCmpt));
