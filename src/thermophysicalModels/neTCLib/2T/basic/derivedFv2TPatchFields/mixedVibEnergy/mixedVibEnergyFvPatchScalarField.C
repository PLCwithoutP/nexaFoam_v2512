...
Foam::mixedVibEnergyFvPatchScalarField::
mixedVibEnergyFvPatchScalarField
(
    const fvPatch& p,
...
mixedVibEnergyFvPatchScalarField
(
    const mixedVibEnergyFvPatchScalarField& ptf,
    const fvPatch& p,
    const DimensionedField<scalar, volMesh>& iF,
...
mixedVibEnergyFvPatchScalarField
(
    const mixedVibEnergyFvPatchScalarField& tppsf
)
:
...
void Foam::mixedVibEnergyFvPatchScalarField::updateCoeffs()
{
    if (updated())
    {
        return;
    ...
    refValue()      = thermo2T.eVib(pw, TTRw, TVw.refValue(), theta, patchi);
    refGrad() = thermo2T.CvVib(pw, TTRw, TVw, theta, patchi)*TVw.refGrad()
    + patch().deltaCoeffs()*
        (
...
void Foam::mixedVibEnergyFvPatchScalarField::manipulateMatrix
(
...
)
{
    ...
    const mixedFvPatchField<scalar>& fPatch =
        refCast<const mixedFvPatchField>(thermo2T.TVib().boundaryField()[index]);
