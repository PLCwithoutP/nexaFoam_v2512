...
void Foam::smoluchowskiJumpT::updateCoeffs()
{
    ...
    Field<scalar> ppsi(patch().size());
    Field<scalar> palpha(patch().size());
    forAll(fc, i)
    {
        pmu[i]    = muInt[fc[i]];
        ppsi[i]   = psiInt[fc[i]];
        palpha[i] = alphaInt[fc[i]];
    }

    // gamma() IS a full volScalarField -> boundary slice is valid
    const Field<scalar> gamma(thermo.gamma()().boundaryField()[patchi]);

    const scalarField& rhoInt = db().lookupObject<volScalarField>(rhoName_)();
    Field<scalar> prho(patch().size());
    forAll(fc, i) prho[i] = rhoInt[fc[i]];
