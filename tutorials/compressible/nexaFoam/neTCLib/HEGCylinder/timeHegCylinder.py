#!/usr/bin/env python3
"""
heg_time_surface_plots.py

Time-dependent surface distributions for the nexaFoam HEG air cylinder, in the
convergence-watching style of the double-cone p/q snapshots: several time
snapshots overlaid with a turbo colourmap so you can watch the surface march
toward steady state.

Figures produced, each vs. circumferential angle theta:
...
theta runs 0 deg (windward stagnation) -> 180 deg (leeward). The full half
cylinder is shown on purpose, so the leeward / wall-outlet corner region is
visible while you diagnose convergence. The cylinder centre is recovered by an
algebraic (Kasa) circle fit and theta is built purely from geometry + the known
freestream direction, so it is immune to spurious peaks in the solution
(e.g. a corner blow-up that would hijack a max-pressure anchor).

Usage:
    python heg_time_surface_plots.py
Edit the CONFIG block for your paths / surface name / times / quantities.
"""
...
# =============================== CONFIG ==================================== #
RHO_INF   = 1.547e-3         # kg/m^3   (HEG freestream)
U_INF     = 5956.0           # m/s
FLOW_DIR  = (1.0, 0.0, 0.0)  # freestream velocity direction in the mesh frame
THETA_MAX = 180.0            # half cylinder (show the whole surface)
...
# nexaFoam p is in Pa (-> kPa) and wallHeatFlux in W/m^2 (-> MW/m^2); the FOs
# already normalise Cp/Cf/St, so those are scale 1.0.
QUANTITIES = [
    dict(key="pressure", field="p",            tag="p",  scale=1e-3, take_abs=False,
         ylabel=r"Surface pressure  [kPa]", ylim=None),
    dict(key="heatflux", field="wallHeatFlux", tag="qw", scale=1e-6, take_abs=True,
         ylabel=r"Surface heat flux  [MW/m$^2$]", ylim=None),
    ...
    dict(key="st", field="StantonNumber", tag="St", scale=1.0, take_abs=False,
         ylabel=r"Stanton number  $St$", ylim=None),
]
# =========================================================================== #
...
def main():
    ...
    print(f"[HEG]  q_inf = {q_inf:.6g} Pa  (FOs already normalise Cp/Cf/St)")
    ...
    for q in QUANTITIES:
        ...
        out = f"heg_{q['key']}_theta_time.png"
